    ...
    courses.forEach(course => {
        ...
        if (selectedReportType === 'entryTest') {
            ...
            if (portfolioReportInfo) {
                ...
                if (portfolioReportInfo.url_informe) buttonsHtml += `<a href="${portfolioReportInfo.url_informe}" target="_blank" class="btn">Ver Archivo</a>`;
            } else {
...
function buildDocPortfolioForm(reportData) {
    ...
    const createItemHTML = (item) => {
        let folderUrl = reportData[item.key] || '#';
        if (reportData[item.key]) {
            try {
                const parsed = JSON.parse(reportData[item.key]);
                if (parsed && parsed.url) folderUrl = parsed.url;
            } catch (e) { }
        }
